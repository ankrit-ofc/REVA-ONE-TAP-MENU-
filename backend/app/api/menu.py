...
from app.schemas.menu import CategoryPublic
from app.services import menu_service

router = APIRouter(tags=["menu"])


@router.get("/menu", response_model=list[CategoryPublic])
def get_menu(
    session: Annotated[TableSession, Depends(get_current_session)],
    db: Annotated[Session, Depends(get_db)],
) -> list[CategoryPublic]:
    """
    Returns the full customer-visible menu for the restaurant bound to the
    caller's table session.

    restaurant_id is derived exclusively from the validated session — never
    from a query param or request body.
    """
    return menu_service.get_customer_menu(db, session.restaurant_id)
